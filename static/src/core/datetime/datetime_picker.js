...
const toDateItem = ({ isOutOfRange = false, isValid = true, label, range, extraClass }) => ({
    ...
    isOutOfRange,
    isValid,
    label: String(range[0].jalaliDate[label]),
    range,
    extraClass,
});
...
const toWeekItem = (weekDayItems) => ({
    number: weekDayItems[3].range[0].jalaliDate.weekNumber,
    days: weekDayItems,
});
...
const PRECISION_LEVELS = new Map()
    .set("days", {
        ...
        getTitle: (date, { additionalMonth }) => {
            const titles = [`${date.jalaliDate.monthLong} ${date.jalaliDate.yearFa}`]
            if (additionalMonth) {
                const next = date.jalaliAddMonths(1)
                titles.push(`${next.jalaliDate.monthLong} ${next.jalaliDate.yearFa}`)
            }
            return titles;
        },
        getItems: (
            ...
            if (additionalMonth) {
                startDates.push(date.jalaliAddMonths(1))
            }
            return startDates.map((date, i) => {
                const monthRange = [date.jalaliStartOfMonth(), date.jalaliEndOfMonth()];
                /** @type {WeekItem[]} */
                const weeks = [];
